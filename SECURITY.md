...
- **Malicious configuration causing unsafe behavior.** Config values are rendered into the status bar; a crafted value that escapes the intended output or causes a crash would be a bug worth reporting.
...
- **Dependency vulnerabilities.** `cargo audit` runs on every push, but an advisory we have not noticed is worth flagging.
- **Supply chain issues in the release pipeline.** The workflows publish the wasm binary people install; problems there affect everyone downstream.

## What is not in scope

- **Zellij or zjstatus vulnerabilities.** Report those to their projects: [zellij][zellij-security], [zjstatus][zjstatus-repo].
