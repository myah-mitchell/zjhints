...
- **Grouped concepts that no single action describes**: the four resize directions as one `resize`, or Session mode's plugin launchers.
...
Nothing about it is fixed: every entry is a normal hint with an [id](labels.md#ids), so it can be relabeled with `label_<id>`, moved with [`hint_order`](ordering.md#hint-order), or hidden with an empty label.
...
With it on, every *other* keybinding enabled in the current mode is found and appended after the curated list. Nothing your config binds is left out: custom binds appear, and so do the many defaults the curated list omits (the `Alt-*` quick keys, `lock` in Normal mode), along with modes like Locked and Tmux that have no curated entries at all.

Keys resolving to the same [id](labels.md#labels) become a single hint, so the four directional focus keys collapse into one `focus` rather than four entries.
...
Everything else works the same either way: discovered hints have [ids](labels.md#ids) exactly like curated ones, so they relabel, reorder and hide identically.

### Shared bindings

Zellij's default config binds the mode switches (`pane`, `tab`, `session`, …) in every non-locked mode via `shared_except` groups. Discovering those in each mode means every mode re-lists the same globals, which is mostly noise.

`hide_shared_hints` (default `true`) suppresses any discovered binding whose key maps to the same action in the base mode, so each mode shows only what is new in it. It has no effect while you are *in* the base mode (Normal still lists the globals), and it respects a non-Normal `default_mode` such as `locked`.
