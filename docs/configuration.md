...
- `key_format`: Format string for the keybinding portion of each hint (default: unset, uses the theme palette)
- `desc_format`: Format string for the description portion of each hint (default: unset, uses the theme palette)
...
- `mode_format`: Format string for the mode prefix, with a `{mode}` placeholder (default: unset, uses the theme palette); see [Standing alone](nested-sessions.md#standing-alone)
- `mode_format_<mode>`: The same, for one mode only, e.g. `mode_format_normal`, `mode_format_locked` (default: unset, falls back to `mode_format`); see [Standing alone](nested-sessions.md#standing-alone)
