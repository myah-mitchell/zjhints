...
detaching and reattaching does not reload, so the change simply will not appear.
...
does not recognize is silently **dropped**, and the binding arrives truncated.
...
exist because of Zellij behavior that is invisible from the code.
